...
export class NavigationBarUIStore extends EduUIStoreBase {
  ...
  get actions(): EduNavAction<EduNavRecordActionPayload | undefined>[] {
    ...
    const teachExitAction: EduNavAction<EduNavRecordActionPayload | undefined>[] = [
      exitAction,
    ]

    const teacherActions: EduNavAction<EduNavRecordActionPayload | undefined>[] = [
      exitAction,
      // {
      //   id: 'Record',
    ...
    if (EduClassroomConfig.shared.sessionInfo.role === EduRoleTypeEnum.teacher) {
      console.log(teachExitAction, 'teachExitAction')
      if (!isInSubRoom) {
        actions = actions.concat(teachExitAction);
      }
      // actions = actions.concat(teacherMediaActions); //// this is mic and recorder
    }

    if (EduClassroomConfig.shared.sessionInfo.role === EduRoleTypeEnum.student) {
      // actions = studentMediaActions.concat(actions);
      console.log(studentMediaActions, 'actions123')
      if (isInSubRoom) {

...
export class EduShareUIStore {
  ...
  private _classroomMinimumSize = { width: 1600, height: 768 };
  ...
  updateClassroomViewportSize() {
    ...
    runInAction(() => {
      ...
      this.classroomViewportSize.h5Width = window.innerWidth;
      this.classroomViewportSize.h5Height = window.innerHeight;
    });
  }
  @bound
  ...
  @bound
  moveWindowAlignToWindow(
    windowID: WindowID,
    windowIDAlignTo: WindowID,
    options: Record<string, string | number | boolean>,
  ) {
    sendToMainProcess(ChannelType.MoveWindowAlignToWindow, windowID, windowIDAlignTo, options);
  }

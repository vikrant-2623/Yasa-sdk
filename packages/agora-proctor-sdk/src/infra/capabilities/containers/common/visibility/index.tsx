import { useUIConfig } from '@proctor/infra/hooks/ui-config';
import { FcrUIConfig } from '@proctor/infra/types/config';
...
export const visibilityListItemControl = <T extends unknown>(
  Com: FC<T>,
  check: ListItemConditionCheck<T>,
): FC<T> => {
  const hoc: FC<T> = (props) => {
    const uiConfig = useUIConfig();
